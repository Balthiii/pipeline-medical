...
/** Calcul de statistiques descriptives par patient et par service, à partir
  * de mesures nettoyées et transformées (`MesureEnrichie`). Toutes les
  * agrégations utilisent `fold`/`foldLeft` plutôt que des variables
  * mutables ; seul le tri nécessaire à la médiane utilise `sorted`, une
  * opération intrinsèquement séquentielle qui ne s'exprime pas naturellement
  * en `fold`.
  */
object Statistiques:
  ...
  private def ecartType(valeurs: List[Double], moy: Double): Double =
    val sommeCarresEcarts = valeurs.foldLeft(0.0)((acc, v) => acc + math.pow(v - moy, 2))
  ...
  /** Statistiques descriptives d'un paramètre vital à partir des valeurs
    * présentes uniquement (les `None` d'origine ont déjà été écartés en
    * amont par `.flatMap` avant l'appel à cette fonction).
    */
  ...
  private def statsChamp[A](mesuresEnrichies: List[MesureEnrichie], champ: MesureEnrichie => Option[A])(using
      numeric: Numeric[A]
  ): StatistiquesVitales =
    statistiquesVitales(mesuresEnrichies.flatMap(champ).map(numeric.toDouble))

  private def frequenceCardiaqueStats(ms: List[MesureEnrichie]): StatistiquesVitales =
    statsChamp(ms, _.mesure.frequenceCardiaque)

  private def tensionSystoliqueStats(ms: List[MesureEnrichie]): StatistiquesVitales =
    statsChamp(ms, _.mesure.tensionSystolique)

  private def tensionDiastoliqueStats(ms: List[MesureEnrichie]): StatistiquesVitales =
  ...
  def proportionAlerte(mesuresEnrichies: List[MesureEnrichie]): Double =
    if mesuresEnrichies.isEmpty then 0.0
      ...
      val nbAlertes = mesuresEnrichies.count(_.niveauVigilance == NiveauVigilance.Alerte)
      nbAlertes.toDouble / mesuresEnrichies.length

  /** Statistiques agrégées pour un patient, à partir de ses mesures. */
  def statistiquesPatient(patientId: String, mesuresEnrichies: List[MesureEnrichie]): StatistiquesPatient =
    StatistiquesPatient(
      patientId = patientId,
  ...
  def statistiquesService(service: String, mesuresEnrichies: List[MesureEnrichie]): StatistiquesService =
    StatistiquesService(
      service = service,
      nbMesures = mesuresEnrichies.length,
  ...
  /** Regroupe les mesures par patient (`groupBy`) et calcule les statistiques
    * de chacun.
    */
  def statistiquesParPatient(mesuresEnrichies: List[MesureEnrichie]): Map[String, StatistiquesPatient] =
    mesuresEnrichies
      .groupBy(_.mesure.patientId)
      .map { case (patientId, ms) => patientId -> statistiquesPatient(patientId, ms) }

  /** Regroupe les mesures par service (`groupBy`) et calcule les statistiques
    * de chacun, proportion d'alerte incluse.
    */
  def statistiquesParService(mesuresEnrichies: List[MesureEnrichie]): Map[String, StatistiquesService] =
    mesuresEnrichies
